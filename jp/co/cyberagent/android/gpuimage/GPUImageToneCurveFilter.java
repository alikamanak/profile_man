...
public class GPUImageToneCurveFilter extends GPUImageFilter {
    ...
    class C05511 implements Runnable {
        ...
        public void run() {
            ...
            if (GPUImageToneCurveFilter.this.mRedCurve.size() >= AccessibilityNodeInfoCompat.ACTION_NEXT_AT_MOVEMENT_GRANULARITY && GPUImageToneCurveFilter.this.mGreenCurve.size() >= AccessibilityNodeInfoCompat.ACTION_NEXT_AT_MOVEMENT_GRANULARITY && GPUImageToneCurveFilter.this.mBlueCurve.size() >= AccessibilityNodeInfoCompat.ACTION_NEXT_AT_MOVEMENT_GRANULARITY && GPUImageToneCurveFilter.this.mRgbCompositeCurve.size() >= AccessibilityNodeInfoCompat.ACTION_NEXT_AT_MOVEMENT_GRANULARITY) {
                byte[] toneCurveByteArray = new byte[AccessibilityNodeInfoCompat.ACTION_NEXT_HTML_ELEMENT];
                for (int currentCurveIndex = 0; currentCurveIndex < AccessibilityNodeInfoCompat.ACTION_NEXT_AT_MOVEMENT_GRANULARITY; currentCurveIndex++) {
                    toneCurveByteArray[currentCurveIndex * 4] = (byte) (((int) Math.min(Math.max(((Float) GPUImageToneCurveFilter.this.mRgbCompositeCurve.get(currentCurveIndex)).floatValue() + (((float) currentCurveIndex) + ((Float) GPUImageToneCurveFilter.this.mBlueCurve.get(currentCurveIndex)).floatValue()), 0.0f), 255.0f)) & MotionEventCompat.ACTION_MASK);
                    toneCurveByteArray[(currentCurveIndex * 4) + 1] = (byte) (((int) Math.min(Math.max(((Float) GPUImageToneCurveFilter.this.mRgbCompositeCurve.get(currentCurveIndex)).floatValue() + (((float) currentCurveIndex) + ((Float) GPUImageToneCurveFilter.this.mGreenCurve.get(currentCurveIndex)).floatValue()), 0.0f), 255.0f)) & MotionEventCompat.ACTION_MASK);
                    toneCurveByteArray[(currentCurveIndex * 4) + 2] = (byte) (((int) Math.min(Math.max(((Float) GPUImageToneCurveFilter.this.mRgbCompositeCurve.get(currentCurveIndex)).floatValue() + (((float) currentCurveIndex) + ((Float) GPUImageToneCurveFilter.this.mRedCurve.get(currentCurveIndex)).floatValue()), 0.0f), 255.0f)) & MotionEventCompat.ACTION_MASK);
                    toneCurveByteArray[(currentCurveIndex * 4) + 3] = (byte) -1;
                }
                GLES20.glTexImage2D(3553, 0, 6408, AccessibilityNodeInfoCompat.ACTION_NEXT_AT_MOVEMENT_GRANULARITY, 1, 0, 6408, 5121, ByteBuffer.wrap(toneCurveByteArray));
            }
        }
    }

    /* renamed from: jp.co.cyberagent.android.gpuimage.GPUImageToneCurveFilter.2 */
    class C05522 implements Comparator<PointF> {
        C05522() {
        }
    ...
    private ArrayList<Float> createSplineCurve(PointF[] points) {
        ...
        Arrays.sort(pointsSorted, new C05522());
        ...
        if (lastSplinePoint.x < MotionEventCompat.ACTION_MASK) {
            for (i = lastSplinePoint.x + 1; i <= MotionEventCompat.ACTION_MASK; i++) {
                splinePoints.add(new Point(i, MotionEventCompat.ACTION_MASK));
            }
        }
